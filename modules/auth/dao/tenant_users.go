...
func (dao *TenantUserDao) Restore(ctx context.Context, id int64) error {
	_, err := dao.Context(ctx).Unscoped().Where(dao.query.TenantUser.ID.Eq(id)).UpdateSimple(dao.query.TenantUser.DeletedAt.Null())
	return err
}
