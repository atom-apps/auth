...
type DictionaryGroup struct {
	...
	UserID      string         `gorm:"column:user_id;type:character varying(198);not null;comment:用户ID" json:"user_id"`          // 用户ID
	TenantID    string         `gorm:"column:tenant_id;type:character varying(198);not null;comment:租户ID" json:"tenant_id"`      // 租户ID
	Name        string         `gorm:"column:name;type:character varying(198);not null;comment:字典组名称" json:"name"`               // 字典组名称
	Slug        string         `gorm:"column:slug;type:character varying(120);not null;comment:字典组别名" json:"slug"`               // 字典组别名
